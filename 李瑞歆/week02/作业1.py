...
def classify_text(text, model, char_to_index, vocab_size, max_len, index_to_label):
    ...
    # 把网络切换到评估模式。
    # 作用：关闭 Dropout、BatchNorm 的「训练期行为」，保证推理结果稳定。
    # 必须写，否则模型仍按训练模式跑，输出会带随机性。
    model.eval()
    # 关闭梯度计算，节省显存、加速推理。
    # 因为只是前向传播，不需要反向求导。
    with torch.no_grad():
        # 把刚才构造好的 BoW 向量（形状 (1, vocab_size)）喂给模型。
        # 得到原始 logits，形状 (1, num_classes)，例如 [[-1.23, 4.56, 0.78]]。
        output = model(bow_vector)

    # 在第 1 维（类别维）上取最大值，返回 (max_value, max_index)。
    # 我们只需要索引，所以用 _ 忽略最大值，把 max_index 赋给 predicted_index，例如 tensor([1])。
